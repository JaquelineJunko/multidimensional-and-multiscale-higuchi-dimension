function [ DH_line ] = mmHD_row( data1D, Lmax, kmax)
%{
...
%}
    ...
    for k = 1 : size(L, 2)      % For each vector/box lenght 
        ...
        if (length(newSerie) > 1)
            DH_partial(1,k) = Higuchi_FD(newSerie, kmax);
            total_created_series = total_created_series + 1;    % number of new auxiliary series created
        end

    end             % Loop for each vector lenght

    DH_line = DH_partial; % return the Higuchi dimension on each lenght L

    DH_line(1,end+1) = sum(DH_partial)/total_created_series;     % Compute the Higuchi dimension for the row. (Average Higuchi dimension of each vector of size L)
end
